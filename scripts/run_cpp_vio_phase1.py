...
CPP = REPO / "cpp_vio"
IMG = "vo-cpp-ceres:1"
HELDOUT = REPO / "_vo_synth_impl_run/cache/heldout/vo-synth-heldout-synth1_synth2"
SEQS = ["synth1", "synth2"]
REF = 1.7  # Python reference VO t_err on this clean stereo


def main():
    if not all((HELDOUT / f"seq_{s}" / "gt.txt").exists() for s in SEQS):
        print("missing cached synth stereo held-out:", HELDOUT); return 1
    art = Path(tempfile.mkdtemp(prefix="cppvio1_art_"))
    ...
    print("=" * 70)
    print(f"PHASE-1 C++ stereo VO + Ceres BA: t_err = {overall:.2f}%  per-seq {per}  (ref ~{REF}%)")
    ok = overall < 10.0   # a real VO must be far below the 99% degenerate; near ref is the goal
    print(f"PHASE 1 {'PASS' if ok else 'FAIL'}: "
          f"{'working stereo VO + windowed BA (beats degenerate; '+('~matches' if overall<2.5 else 'above')+' ref)' if ok else 'not a working VO yet — iterate'}")
