...
def mapimg(grid, title, color=cv2.COLORMAP_INFERNO):
    ...
    cv2.putText(im, title, (6, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    return im
...
for f in files:
    ...
    cv2.putText(cam, "camera 0", (6, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 230, 0), 1)
    ...
    cv2.putText(ov, f"TP/FN/FP  IoU {iou:.2f}", (6, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1)
    panel = np.hstack([cam, mapimg(gt, "GT"), mapimg(pred, "agent pred"), ov])
    frames.append(cv2.cvtColor(panel, cv2.COLOR_BGR2RGB))
